...
2013-01-23  1.9.0: Allow editing of plugin settings via the WordPress admin interface so that settings persist after upgrades.
2013-01-13  1.8.2: Fix e-mail text about the author of a post if it has been set by another user. (Thanks Alexander!) Allow editors to manage collaboration e-mail settings in WordPress 3.5+ (required capability is now "manage_categories" instead of "manage_links").
...
// ----------------------------------------------------------------------
// As of version 1.9.0 of this plugin and higher, all settings are configured in 'Settings' > 'Collaboration e-mails' in the WordPress admin panel
// ----------------------------------------------------------------------
...
function pce_pending($pce_newstatus, $pce_oldstatus, $pce_object) {
    global $wpdb, $pce_db_group, $pce_db_collab, $user_identity, $user_email, $user_ID, $_POST;
    
    $pce_blogname = pceFunctionCollection::get_settings( 'blogname' );
    if( !$pce_blogname )
    {
        $pce_blogname = get_option( 'blogname' );
    }
    $pce_fromaddress = pceFunctionCollection::get_settings( 'fromaddress' );
    $pce_fromname = pceFunctionCollection::get_settings( 'fromname' );
    $pce_whoapproved = pceFunctionCollection::get_settings( 'whoapproved' );
    ...
    if( in_array( 'pending', $pce_emails_to_send ) && 'pending' == $pce_newstatus && 'pending' != $pce_oldstatus )
    {
        ...
        $pce_headers .= 'From:' . $admin_email  . $pce_eol;
        ...
        $pce_body = sprintf(__('There is a new post to review, written by %s.', 'peters_collaboration_emails'), $pce_thisuser->display_name) . $pce_eol . $pce_eol;
        // Insert note if applicable
        if(isset($pce_post_note)) {
            $pce_body .= sprintf(__('Accompanying note from %s:', 'peters_collaboration_emails'), $pce_thisuser->display_name) . $pce_eol;
            $pce_body .= $pce_post_note . $pce_eol . $pce_eol;
        }
        $pce_body .= __('Review and publish it here: ', 'peters_collaboration_emails') . get_edit_post_link( $pce_object->ID, false );
    ...
    elseif( in_array( 'approved', $pce_emails_to_send ) && 'pending' == $pce_oldstatus && 'publish' == $pce_newstatus )
    {
    ...
    elseif( in_array( 'future', $pce_emails_to_send ) && 'pending' == $pce_oldstatus && 'future' == $pce_newstatus )
    {
        // Store the ID of the user who approved the post in the database
        update_post_meta( $pce_object->ID, 'approver_user_id', $user_ID );
    ...
    elseif( in_array( 'backtodraft', $pce_emails_to_send ) && 'pending' == $pce_oldstatus && 'draft' == $pce_newstatus )
    {
        ...
        $pce_body .= __('Edit it again here: ', 'peters_collaboration_emails') . get_edit_post_link( $pce_object->ID, false );
    ...
    elseif( ( in_array( 'wentlive', $pce_emails_to_send ) && 'future' == $pce_oldstatus && 'publish' == $pce_newstatus ) ||
            ( in_array( 'private_to_published', $pce_emails_to_send ) && 'private' == $pce_oldstatus && 'publish' == $pce_newstatus ) )
    {
        if( !$pce_fromaddress )
        {
...
function pce_usersoptions($pce_existingmoderators = array(), $pce_contributors_or_moderators, $pce_optionsoutput = true, $pce_numbered = 0) {
    global $wpdb, $pce_moderatorcache;
    ...
    switch ($pce_contributors_or_moderators) {
        case 'contributors':
            $pce_filter_roles = pceFunctionCollection::get_settings( 'contributor_roles' );
            break;
        case 'moderators':
        default:
            $pce_filter_roles = pceFunctionCollection::get_settings( 'moderator_roles' );
            break;
    }
    $delimiter = '';
...
?>
    <div class="wrap">
        ...
        <form name="pce_edit_group" method="post" action="<?php print '?page=' . basename(__FILE__) . '&group=' . $pce_groupid; ?>">
            ...
            <p class="submit"><input type="submit" name="pce_edit_group_submit" class="button" value="<?php _e('Update', 'peters_collaboration_emails'); ?>" /></p>
        </form>
        
        <form name="pce_delete_group" method="post" action="<?php print '?page=' . basename(__FILE__); ?>">
            <p class="submit"><input type="hidden" name="pce_groupid" value="<?php print $pce_groupid;?>"><input type="submit" name="pce_delete_group_submit" class="button" value="<?php _e('Delete this group', 'peters_collaboration_emails'); ?>" /></p>
        </form>
    </div>
...
function pce_mainoptionsmenu()
{
    ...
    elseif( isset( $_POST['pce_settingssubmit'] ) )
    {
        $pce_process_submit = pceFunctionCollection::submit_settings();
    }
    
    // Settings that can be updated
    $pce_settings = pceFunctionCollection::get_settings();
...
?>
    <div class="wrap">
        ...
        <p class="submit"><input type="submit" class="button" name="pce_modsubmit" value="<?php _e('Update', 'peters_collaboration_emails'); ?>" /></p>
        </form>
        ...
        <p class="submit"><input type="submit" class="button" name="pce_rulesubmit" value="<?php _e('Update', 'peters_collaboration_emails'); ?>" /></p>
        </form>
        ...
        <p class="submit"><input type="submit" class="button" name="pce_groupsubmit" value="<?php _e('Update', 'peters_collaboration_emails'); ?>" /></p>
        </form>
        ...
        <p class="submit"><input type="submit" class="button" name="pce_post_type_submit" value="<?php _e('Update', 'peters_collaboration_emails'); ?>" /></p>
        </form>
        ...
        <h4><?php _e('Add post-type-specific moderators', 'peters_collaboration_emails'); ?></h4>
            ...
            ?>
        <p class="submit"><input type="submit" class="button" name="pce_add_post_type_submit" value="<?php _e('Update', 'peters_collaboration_emails'); ?>" /></p>
        </form>        
    </div>
    ...
    <h3><?php _e( 'Customize plugin settings', 'peters_collaboration_emails' ); ?></h3>
    <form name="pce_settingsform" action="<?php print '?page=' . basename(__FILE__); ?>" method="post">
    <table class="widefat">
    <tr>
        <td>
            <p><strong><?php _e( 'The name of your blog, to appear in the title of e-mails. Leave blank to default to the site name within WordPress.', 'peters_collaboration_emails' ); ?></strong></p>
        </td>
        <td>
            <input name="pce_blogname" type="text" size="50" value="<?php print htmlspecialchars( $pce_settings['blogname'] ); ?>" />
        </td>
    </tr>
    <tr>
        <td>
            <p><strong><?php _e( 'E-mail address for the person sending all e-mails. If left blank, the sender is the user performing the action.', 'peters_collaboration_emails' ); ?></strong></p>
        </td>
        <td>
            <input name="pce_fromaddress" type="text" size="50" value="<?php print htmlspecialchars( $pce_settings['fromaddress'] ); ?>" />
        </td>
    </tr>
    <tr>
        <td>
            <p><strong><?php _e( 'Name for the person sending all e-mails. If left blank, the name is of the user performing the action.', 'peters_collaboration_emails' ); ?></strong></p>
        </td>
        <td>
            <input name="pce_fromname" type="text" size="50" value="<?php print htmlspecialchars( $pce_settings['fromname'] ); ?>" />
        </td>
    </tr>
    <tr>
        <td>
            <p><strong><?php _e( 'Let the contributor know who approved his / her post', 'peters_collaboration_emails' ); ?></strong></p>
        </td>
        <td>
            <select name="pce_whoapproved">
                <option value="1"<?php if( $pce_settings['whoapproved'] ) print ' selected="selected"'; ?>><?php _e( 'Yes', 'peters_collaboration_emails' ); ?></option>
                <option value="0"<?php if( !$pce_settings['whoapproved'] ) print ' selected="selected"'; ?>><?php _e( 'No', 'peters_collaboration_emails' ); ?></option>
            </select>
        </td>
    </tr>
    <tr>
        <td>
            <p><strong><?php _e( 'Which roles on your site can only "submit for review"', 'peters_collaboration_emails' ); ?></strong></p>
        </td>
        <td>
            <?php
                $pce_role_names = pceFunctionCollection::get_role_names();
                // Build the option HTML
                foreach( $pce_role_names as $pce_role_name )
                {
                    print '<p><input name="pce_contributor_roles[]" type="checkbox" value="' . $pce_role_name . '"';
                    if( in_array( $pce_role_name, $pce_settings['contributor_roles'] ) )
                    {
                        print ' checked="checked"';
                    }
                    print ' /> ' . $pce_role_name . '</p>';
                }
            ?>
        </td>
    </tr>
    <tr>
        <td>
            <p><strong><?php _e( 'Which roles on your site can approve posts', 'peters_collaboration_emails' ); ?></strong></p>
        </td>
        <td>
            <?php
                $pce_role_names = pceFunctionCollection::get_role_names();
                ...
                foreach( $pce_role_names as $pce_role_name )
                {
                    print '<p><input name="pce_moderator_roles[]" type="checkbox" value="' . $pce_role_name . '"';
                    if( in_array( $pce_role_name, $pce_settings['moderator_roles'] ) )
                    {
                        print ' checked="checked"';
                    }
                    print ' /> ' . $pce_role_name . '</p>';
    ...
    <tr>
        <td>
            <p><strong><?php _e( 'Which e-mails to send', 'peters_collaboration_emails' ); ?></strong></p>
        </td>
        <td>
            <?php
                $pce_emails_to_send_options = pceFunctionCollection::get_settings( 'emails_to_send_options' );
                // Build the option HTML
                foreach( $pce_emails_to_send_options as $pce_emails_to_send_option_identifier => $pce_emails_to_send_option_description )
                {
                    print '<p><input name="pce_emails_to_send[]" type="checkbox" value="' . $pce_emails_to_send_option_identifier . '"';
                    if( in_array( $pce_emails_to_send_option_identifier, $pce_settings['emails_to_send'] ) )
                    {
                        print ' checked="checked"';
                    }
                    print ' /> ' . $pce_emails_to_send_option_description . '</p>';
                }
            ?>
        </td>
    </tr>
    <tr>
        <td>
            <p><strong><?php _e( 'Permission level required to edit collaboration e-mail settings', 'peters_collaboration_emails' ); ?></strong></p>
        </td>
        <td>
            <select name="pce_required_capability">
                <?php
                    $pce_level_names = pceFunctionCollection::get_level_names();
                    // Build the option HTML
                    foreach( $pce_level_names as $pce_level_name )
                    {
                        print '<option value="' . $pce_level_name . '"';
                        if( $pce_level_name == $pce_settings['required_capability'] )
                        {
                            print ' selected="selected"';
                        }
                        print '>' . $pce_level_name . '</option>';
                    }
                ?>
            </select>
        </td>
    ...
    <p class="submit"><input name="pce_settingssubmit" type="submit" class="button" value="<?php _e( 'Update', 'peters_collaboration_emails' ); ?>" /></p>
    </form>
<?php
}
...
{
    $pce_required_capability = pceFunctionCollection::get_settings( 'required_capability' );
...
class pceFunctionCollection
{
    ...
    */
    static function get_settings( $setting=false )
    {
        ...
        // false = get_option( 'blogname' );
        $pce_settings['blogname'] = false;
        ...
        $pce_settings['fromaddress'] = false;
        ...
        $pce_settings['contributor_roles'] = array( 'contributor' );

        // Which roles on your site can approve posts
        // Typically you do not have to edit this unless you have custom roles and capabilities
        $pce_settings['moderator_roles'] = array( 'administrator', 'editor' );

        /*
          Which e-mails to send
          'pending' = Set to pending for the first time
          'approved' = Goes from pending to immediately published
          'future' = Goes from pending to approved at a set time in the future
          'backtodraft' = Goes back to draft status
          'wentlive' = Goes from scheduled to be published to actually published
          'private_to_published' = Goes from private to published
          'edited' = Edited and commented on (using the Peter's Post Notes plugin) by someone who is not the author, without having changed the status
                     This applies no matter what the status is, so it works for example for pending and published posts
        */
        // This is the list of enabled e-mails
        $pce_settings['emails_to_send'] = array( 'pending', 'approved', 'future', 'backtodraft', 'wentlive', 'private_to_published', 'edited' );
        ...
        $pce_settings['emails_to_send_options']['wentlive'] = __( 'Goes from scheduled to be published to actually published', 'peters_collaboration_emails' );
        $pce_settings['emails_to_send_options']['private_to_published'] = __( 'Goes from private to published', 'peters_collaboration_emails' );
        $pce_settings['emails_to_send_options']['edited'] = __( 'Edited and commented on (using the Peter\'s Post Notes plugin) by someone who is not the author, without having changed the status', 'peters_collaboration_emails' );
        ...
        $pce_settings['required_capability'] = 'manage_categories';
        
        $pce_settings_from_options_table = pceFunctionCollection::get_settings_from_options_table();
        ...
        foreach( $pce_settings as $setting_name => $setting_value )
        {
            if( isset( $pce_settings_from_options_table[$setting_name] ) )
            {
                $pce_settings[$setting_name] = $pce_settings_from_options_table[$setting_name];
            }
        }
        if( !$setting )
        {
        ...
        elseif( $setting && isset( $pce_settings[$setting] ) )
        {
            return $pce_settings[$setting];
        }
        else
        {
            return false;
    ...
    {
        return get_option( 'pce_settings', array() );
    }
    static function set_setting( $setting = false, $value = false )
    {
        if( $setting )
        {
            $current_settings = pceFunctionCollection::get_settings();
            if( $current_settings )
            {
                $current_settings[$setting] = $value;
                update_option( 'pce_settings', $current_settings );
            }
        }
    }
    ...
    static function submit_settings()
    {
        ...
        foreach( $pce_settings as $setting_name => $setting_value )
        {
            if( isset( $_POST['pce_' . $setting_name] ) )
            {
                $pce_settings[$setting_name] = $_POST['pce_' . $setting_name];
            }
            // Special settings for checkboxes: in case nothing is submitted, we clear them
            elseif( in_array( $setting_name, array( 'contributor_roles', 'moderator_roles', 'emails_to_send' ) ) )
            {
                $pce_settings[$setting_name] = array();
            }
        }
        update_option( 'pce_settings', $pce_settings );
        return array( 'success', __( 'Successfully updated plugin settings', 'peters_collaboration_emails' ) );
    }
    ...
    static function pce_get_post_type_taxonomies()
    {
    ...
    static function pce_get_taxonomy_terms()
    {
    ...
    static function pce_add_post_type_submit()
    {
        global $wpdb, $pce_db_collabrules;
    ...
    static function pce_post_type_submit()
    {
        global $wpdb, $pce_db_collabrules;
    ...
    static function pce_delete_post_type_rule( $rule_id )
    {
        global $wpdb, $pce_db_collabrules;
        ...
        {                
            ...
            else
            {
                ...
                print "\n" . '<p class="submit"><input type="submit" class="button" name="pce_delete_post_type_rule_yes" value="' . __( 'Yes', 'peters_collaboration_emails') . '" /></p>';
                print "\n" . '</form>';
                print "\n" . '<form method="post" action="?page=' . basename(__FILE__) . '">';
                print "\n" . '<p class="submit"><input type="submit" class="button" value="' . __( 'No, go back', 'peters_collaboration_emails' ) . '" /></p>';
                print "\n" . '</form>';
            }
        }
        else
    ...
    static function post_type_rule_exists( $rule_id )
    {
        global $wpdb, $pce_db_collabrules;
    ...
    static function create_post_type_table()
    {
        global $wpdb, $pce_db_collabrules;
        if( $wpdb->get_var( 'SHOW TABLES LIKE \'' . $pce_db_collabrules . '\'' ) != $pce_db_collabrules )
        {
